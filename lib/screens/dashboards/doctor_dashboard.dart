...
class _DoctorDashboardState extends State<DoctorDashboard> {
  ...
  if (_assignedPatients.isEmpty) return;
  setState(() => _loadingNotification = true);
  try {
    List<Map<String, dynamic>> allAlerts = [];
    for (final patient in _assignedPatients) {
      ...
      try {
        ...
        for (final alert in alerts) {
          alert['_patient_name'] = patient['full_name'] ?? 'Unknown';
        }
    ...
    if (mounted) {
      setState(() {
        _latestAlert = allAlerts.isNotEmpty ? allAlerts.first : null;
  ...
  void _loadAssignedPatients() async {
    try {
      ...
      _loadLatestNotification(); // ← add this
      }
    } catch (e) {
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          _buildTabButton(2, 'Notifications', Icons.notifications),
        ...
        children: [
          ...
          TextField(
                ...
                if (query.isEmpty) {
                  ...
                  _pharmacySearchResults =
                      _allPharmacies.where((pharmacy) {
                        final name = (pharmacy['pharmacy_name'] ?? '')
                            .toString()
                            .toLowerCase();
                        final district = (pharmacy['district'] ?? '')
                            .toString()
                            .toLowerCase();
                        final province = (pharmacy['province'] ?? '')
                            .toString()
                            .toLowerCase();
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: _isPharmacySearching
                  ? const SizedBox(
                      width: 20,
                      height: 20,
                      child: Padding(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      pharmacy['pharmacy_name'] ?? 'Unknown',
                                      style: const TextStyle(
                                        fontWeight: FontWeight.w600,
                                        fontSize: 14,
                                        color: AppTheme.textPrimaryColor,
          ...
          if (_prescriptions.isEmpty)
            Center(
              child: Padding(
                padding: const EdgeInsets.all(32),
                child: Text('No prescriptions sent yet',
                    style: TextStyle(
                        color: AppTheme.textSecondaryColor, fontSize: 14)),
              ),
            )
      ...
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            value: _selectedPatientForPrescription != null
                ? (_selectedPatientForPrescription!['patient_id'] ??
                        _selectedPatientForPrescription!['id'])
                    ?.toString()
                : null,
            items: _assignedPatients.map((patient) {
              ...
              return DropdownMenuItem<String>(
                value: id,
                child: Text(patient['full_name'] ?? 'Unknown Patient',
                    style: const TextStyle(fontWeight: FontWeight.w600)),
              );
            }).toList(),
            onChanged: (String? newValue) {
              if (newValue != null) {
                final match = _assignedPatients.firstWhere(
                  (p) =>
                      (p['patient_id'] ?? p['id'])?.toString() == newValue,
                  orElse: () => {},
                );
                if (match.isNotEmpty) {
                  setState(() => _selectedPatientForPrescription = match);
                }
              }
            },
            decoration: InputDecoration(
          ...
          Row(
            ...
            children: [
              ElevatedButton(
                onPressed: (_selectedPatientForPrescription == null ||
                        ...
                        try {
                          ...
                          final rawPharmacyId =
                              _selectedPharmacyForPrescription!['pharmacy_id'] ??
                                  _selectedPharmacyForPrescription!['id'];

                          if (rawPatientId == null || rawPharmacyId == null) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                    'Error: Could not resolve patient or pharmacy ID'),
                          ...
                          final notes = _prescriptionNotesController.text
                                  .trim()
                                  .isNotEmpty
                          ...
                          await ApiService.sendPrescription(
                            patientId: rawPatientId.toString(),
                            pharmacyId: rawPharmacyId.toString(),
  ...
  Widget _buildPrescriptionCard(Map<String, dynamic> prescription) {
    ...
    final medicinesDisplay = medicines is List
        ? medicines.map((m) {
                  ...
                  children: [
                    const Text('💊 Prescription:',
                        style: TextStyle(
                            ...
                            color: AppTheme.textPrimaryColor)),
                    const SizedBox(height: 4),
                    Text(medicinesDisplay,
                        style: const TextStyle(
                            fontSize: 11, color: AppTheme.textPrimaryColor),
                        maxLines: 3,
                        overflow: TextOverflow.ellipsis),
                  ],
                ),
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    onTap: () {
                      ...
                      if (patientId != null) {
                        _loadPatientHistory(patientId.toString());
                      }
                    },
                  ),
                );
      ...
      child: Column(
        ...
        children: [
          ...
          if (_loadingPatientHistory)
            ...
            Column(
              children: [
                if ((_patientHistory!['seizure_events'] as List?)
                        ?.isNotEmpty ??
                    false)
                  _buildEventSection(
                      'Seizure Events',
                ...
                if ((_patientHistory!['cardiac_events'] as List?)
                        ?.isNotEmpty ??
                    false)
                  _buildEventSection(
                      'Cardiac Events',
                ...
                if ((_patientHistory!['alerts'] as List?)?.isNotEmpty ??
                    false)
                  _buildEventSection(
                      'Alerts',
                      Colors.blue[100]!,
  ...
  Widget _buildEventSection(
      String title, Color backgroundColor, Color borderColor, List events) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(title,
        ...
        ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final match = urlRegex.firstMatch(event['message']);

            if (match == null) {
              return Text(
                'Msg: ${event['message']}',
                style: TextStyle(fontSize: 12, color: borderColor.withOpacity(0.7),),
              );
            }

            final url = match.group(0)!;
            final textBefore = event['message'].substring(0, match.start);
            return Card(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildEventDetails(event), // ✅ comma added above
                  ],
                ),
              ),
            );
  ...
  Widget _buildEventDetails(Map<String, dynamic> event) {
    ...
    if (event.containsKey('alert_type')) details['Type'] = event['alert_type'];
  ...
  String _formatEventTitle(String type, Map<String, dynamic> event) {
    if (type == 'Alerts') return event['alert_type'] ?? 'Alert';
    if (type == 'Seizure Events') return 'Seizure Event';
    if (type == 'Cardiac Events') return 'Cardiac Event';
    return type;
  }
    ...
    child: Column(
      ...
      children: [
        ...
        if (_loadingNotification)
          ...
          _buildLatestAlertCard(_latestAlert!),
      ],
    ),
  );
}
...
Widget _buildLatestAlertCard(Map<String, dynamic> alert) {
  ...
  final patientName = alert['_patient_name'] ?? 'Unknown Patient';
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildInfoRow(
                      'License Number', _doctorData['license_number'] ?? ''),
                  _buildInfoRow(
                      'Hospital/Clinic', _doctorData['hospital_clinic'] ?? ''),
                  const SizedBox(height: 16),
                  SizedBox(
  ...
  void _showEditDialog() {
    final fullNameController =
        TextEditingController(text: _doctorData['full_name'] ?? '');
    final phoneController =
        TextEditingController(text: _doctorData['phone_number'] ?? '');
    final specializationController =
        TextEditingController(text: _doctorData['specialization'] ?? '');
    final licenseController =
        TextEditingController(text: _doctorData['license_number'] ?? '');
    final hospitalController =
        TextEditingController(text: _doctorData['hospital_clinic'] ?? '');
